...
public class NeedleHelmetModel<T extends AvatarRenderState> extends HumanoidModel<T> {
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(Identifier.fromNamespaceAndPath(AracneMod.MODID, "needle_helmet"), "main");
	private final ModelPart head;
	private final ModelPart horn;

	public NeedleHelmetModel(ModelPart root) {
        super(root);
        this.head = root.getChild("head");
		this.horn = this.head.getChild("horn");
	}
	public void setPartVisibility(NeedleHelmetModel<?> model, EquipmentSlot slot) {
		this.allParts().forEach(modelPart-> modelPart.visible = false);
		switch (slot) {
			case CHEST -> {
				body.visible = true;
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition head = partdefinition.addOrReplaceChild("head", CubeListBuilder.create().texOffs(0, 0).addBox(-4.0F, -7.0F, -4.0F, 8.0F, 8.0F, 8.0F, new CubeDeformation(0.75F).extend(0.5F)), PartPose.offset(0.0F, 0.0F, 0.0F));
		PartDefinition horn = head.addOrReplaceChild("horn", CubeListBuilder.create().texOffs(26, 11).addBox(-1.5F, -2.5125F, -2.25F, 3.0F, 5.0F, 16.0F, new CubeDeformation(0.0F))
				.texOffs(24, -16).addBox(0.0F, -2.4875F, -15.25F, 0.0F, 5.0F, 19.0F, new CubeDeformation(0.0F)), PartPose.offset(0.0F, -5.1625F, -17.35F));
