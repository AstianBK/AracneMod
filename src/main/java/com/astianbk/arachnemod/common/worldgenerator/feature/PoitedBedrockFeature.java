...
public class PoitedBedrockFeature extends Feature<SpeleothemClusterConfiguration> {
    ...
    private void placeColumn(WorldGenLevel level, RandomSource random, BlockPos pos, int dx, int dz, float chanceOfWater, double chanceOfStalagmiteOrStalactite, int clusterHeight, float density, SpeleothemClusterConfiguration config) {
        ...
        if (!baseColumn.isEmpty()) {
            ...
            if (!ceiling.isEmpty() || !baseFloor.isEmpty()) {
                ...
                if (floor.isPresent()) {
                    BedrockUtils.growPointedDripstone(level, pos.atY(floor.getAsInt() + 1), Direction.UP, actualStalagmiteHeight, mergeTips);
                }
            }
        }
    }
    ...
    private boolean canPlacePool(WorldGenLevel level, BlockPos pos) {
        ...
        if (!state.is(Blocks.WATER) && !state.is(NRegistry.POINTED_BEDROCK_BLOCK.get())) {
    ...
    private void replaceBlocksWithDripstoneBlocks(WorldGenLevel level, BlockPos firstPos, int maxCount, Direction direction) {
        ...
        for (int i = 0; i < maxCount; i++) {
            if (!BedrockUtils.placeDripstoneBlockIfPossible(level, pos)) {
                return;
            }
